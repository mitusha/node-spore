...
Client.prototype = {
     ...
     * middleware, params, payload, timeout, callback
     * middleware, payload, callback
     * middleware, payload, timeout, callback
     * middleware, params, callback
     * middleware, params, timeout, callback
     * middleware, callback
     * middleware, timeout, callback
     * params, payload, callback
     * params, payload, timeout, callback
     * payload, callback
     * payload, timeout, callback
     * params, callback
    ...
    _call: function(methodName, args) {
        // TODO properly parse args to handle timeout
        var timeout = 1000;
        ...
        if (this._isValidCall(method, callback, params, payload)) {
            ...
            this._callRequestMiddlewares(middlewares, method, request, [], callback, timeout);
        }
    },
    /**
     * Call each middlewares with callback named 'next' for asynchronous processing.
     */
    _callRequestMiddlewares: function(middlewares, method, request, response_middlewares, callback, timeout) {
        var that = this;
        try {
            var middleware = middlewares.shift();
            if (!middleware) {
                return this._makeRequest(request, callback, response_middlewares, timeout);
            }
            if (middleware.cond(method, request)) {
                var middleware_callback = function(retVal) {
                    ...
                    that._callRequestMiddlewares(middlewares, method, request, response_middlewares, callback, timeout);
                };
                middleware.fun(method, request, middleware_callback);
            } else {
                this._callRequestMiddlewares(middlewares, method, request, response_middlewares, callback, timeout);
            }
        } catch (e) {
            callback(e, null);
        }
     ...
     * Make http request
     */
    _makeRequest: function(request, callback, response_middlewares, timeout) {
        var that = this;
        request.finalize(timeout, function(err, res) {
